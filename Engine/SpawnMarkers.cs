...
{
    // Renders colored line segments in world space — used for spawn state indicators
    // (selected / placeholder / dirty). Uses a single persistent DynamicDraw VBO that's
    // partially rewritten each frame via BufferSubData, so per-frame cost is trivial.
    // Not tied to spawn semantics directly — SpawnManager builds the line list.
    public class SpawnMarkers
    {
        // Vertex layout: 3 floats position + 4 floats color = 7 floats.
        // 4096 lines × 2 verts × 7 floats = 57344 floats reserved.
...
");
        ...
        // Overwrites the marker line set. Each entry is (start, end, color-with-alpha).
        // Caller controls which spawns produce lines and what colors they get.
